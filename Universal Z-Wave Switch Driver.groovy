...
metadata {
    definition (name: "Universal Zwave Plus Switch", namespace: "jvm", author:"jvm") 
	{
...
void updateZwaveParamters() {
    ...
    state.zwaveParameters?.each { param, data ->
		...
		if(logEnable) log.debug "Getting configuration information for parameter ${param} with data ${data}."
		if(logEnable) log.debug "parameterNumber: ${param.toInteger()}, size: ${data.parameterSize}, scaledConfigurationValue: ${settings[data.input.name]}"
...
void zwaveEvent(hubitat.zwave.commands.configurationv1.ConfigurationReport cmd) {
	if (logEnable) log.debug "Received Configuration V1 Report of ${cmd} with scaled value ${cmd.scaledConfigurationValue}."

    if(state.zwaveParameters[cmd.parameterNumber]) {
        Map configParam=state.zwaveParameters[cmd.parameterNumber]
		if (logEnable) log.debug "Processing v1 configuration report for ${configParam}."
...
void zwaveEvent(hubitat.zwave.commands.configurationv2.ConfigurationReport cmd) {
	if (logEnable) log.debug "Received Configuration V2 Report of ${cmd} with scaled value ${cmd.scaledConfigurationValue}."
	
    if(state.zwaveParameters[cmd.parameterNumber]) {
        Map configParam=state.zwaveParameters[cmd.parameterNumber]
		if (logEnable) log.debug "Processing v2 configuration report for ${configParam}."
...
void pollConfigs() {
    ...
    state.zwaveParameters?.each{ param, data ->
		...
		if(logEnable) log.debug "Polling for ${device.displayName} parameter # ${param}."

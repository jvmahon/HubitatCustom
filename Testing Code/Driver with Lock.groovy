...
metadata {
	definition (name: "[Alpha] Test Lock Setup",namespace: "jvm", author: "jvm") {
...
{
	log.debug "Sensor Multilevel Report is: " + cmd
...
void processNotificationReport(cmd)
{
	...
	switch (cmd.notificationType as Integer)
	{
		case 0x01: // Smoke Alarm
			processSmokeAlarmNotification(cmd)
			break 
		case 0x05: // Water Alarm
			processWaterAlarmNotification(cmd)
			break
		case 0x06: // Locks and entry
			processLockNotifications(cmd)
			break
		case 0x07: // Motion Detectors
			processHomeSecurityNotification(cmd)
			break
		default :
...
void processSmokeAlarmNotification(cmd)
{

	switch (cmd.event as Integer)
	{
		case 0x00: // Status Idle
			if (logEnable) log.debug "Smoke Alarm Notification, Status Idle."
			break
		case 0x01: // Smoke detected (location provided)
			if (logEnable) log.debug "Smoke Alarm Notification, Smoke detected (location provided)."
			break
		case 0x02: // Smoke detected
			if (logEnable) log.debug "Smoke Alarm Notification, Smoke detected."
			break
		case 0xFE: // Unknown Event / State
			if (logEnable) log.debug "Smoke Alarm Notification, Unknown Event / State."
		default :
			log.warn "For device ${device.displayName}, Received a Notification Report with type: ${cmd.notificationType}, which is a type not processed by this driver."
	}
}
void processWaterAlarmNotification(cmd)
{

	switch (cmd.event as Integer)
	{
		case 0x00: // Status Idle
			if (logEnable) log.debug "Water Alarm Notification, Status Idle."
			break
		case 0x01: // Water leak detected (location provided)
			if (logEnable) log.debug "Water Alarm Notification, Water leak detected (location provided)."
			break
		case 0x02: // Water leak detected
			if (logEnable) log.debug "Water Alarm Notification, Water leak detected."
			break
		case 0xFE: // Unknown Event / State
			if (logEnable) log.debug "Water Alarm Notification, Unknown Event / State."
		default :
			log.warn "For device ${device.displayName}, Received a Notification Report with type: ${cmd.notificationType}, which is a type not processed by this driver."
	}
}
void processHomeSecurityNotification(cmd)
{
	switch (cmd.event as Integer)
	{
		case 0x00: // Status Idle
			if (logEnable) log.debug "Home Security Notification, Status Idle."
			break
		case 0x03: // Tampering, prouct cover removed
			if (logEnable) log.debug "Home Security Notification, Tampering, prouct cover removed."
			break
		case 0x04: // Tampering, invalid code
			if (logEnable) log.debug "Home Security Notification, Tampering, invalid code."
			break
		case 0x07: // Motion Detection (location provided)
			if (logEnable) log.debug "Home Security Notification, Motion Detection (location provided)."
			break
		case 0x08: // Motion Detection
			if (logEnable) log.debug "Home Security Notification, Motion Detection."
			break
		case 0x09: // Tampering (Product Moved)
			if (logEnable) log.debug "Home Security Notification, Tampering (Product Moved)."
			break
		case 0xFE: // Unknown Event
		default :
			if (logEnable) log.debug "Home Security Notification, Unknown Event."
